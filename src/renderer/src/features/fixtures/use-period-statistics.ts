...
import { readFixturePeriodStatistics, writeFixturePeriodStatisticsRefresh } from '@/data/db'
...
export function usePeriodStatistics(
  ...
  enabled: boolean,
  live: boolean
): RefreshableQuery<Awaited<ReturnType<typeof readFixturePeriodStatistics>>> {
  const cached = useScopedLiveQuery(
    () => (fixtureId === null ? Promise.resolve(null) : readFixturePeriodStatistics(fixtureId)),
    [fixtureId]
  )
  ...
  const refresh = useCallback(async () => {
    ...
    await runRefresh(
      () => refreshPeriodStatistics(fixtureId),
...
export function invalidatePeriodStatisticsRefreshes(): void {
  generation += 1
  requests.clear()
}

async function refreshPeriodStatistics(fixtureId: number): Promise<void> {
  const active = requests.get(fixtureId)
  if (active?.generation === generation) return active.promise
  const startedGeneration = generation
  const promise = (async () => {
    const result = await window.halfspace.sportmonks.refreshFixturePeriodStatistics({ fixtureId })
    if (startedGeneration !== generation) return
    if (!result.ok) throw new Error(result.error.message)
    await writeFixturePeriodStatisticsRefresh(fixtureId, result.data)
  })()
